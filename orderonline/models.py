...
class MenuItem(models.Model):
    ...
    ingredients = models.ManyToManyField(
        'Ingredient', through='MenuItemIngredient')
    included_items = models.ManyToManyField(
        'self',
        through='MenuItemIncludedItem',
        related_name='included_in_items',
        blank=True
    )
...
class MenuItemIngredient(models.Model):
    ...
    option = models.ForeignKey(
        'IngredientOption', null=True, blank=True, on_delete=models.SET_NULL)
...
class MenuItemIncludedItem(models.Model):
    ...
    included_item = models.ForeignKey(
        'MenuItem', related_name='included_in', on_delete=models.CASCADE)
    price = models.DecimalField(max_digits=6, decimal_places=2)
